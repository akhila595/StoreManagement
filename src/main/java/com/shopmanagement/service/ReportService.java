...
public class ReportService {
    ...
    public List<TopSellingProductDTO> getTopSellingProducts(LocalDate startDate, LocalDate endDate, int limit) {
        ...
        for (SaleItem sale : sales) {
            String productName = sale.getProductVariant().getProduct().getProductName();
            productQtyMap.put(productName, productQtyMap.getOrDefault(productName, 0) + sale.getQuantity());
        }

        return productQtyMap.entrySet().stream()
                .sorted((a, b) -> b.getValue().compareTo(a.getValue()))
                .limit(limit)
                .map(e -> new TopSellingProductDTO(e.getKey(), e.getValue()))
